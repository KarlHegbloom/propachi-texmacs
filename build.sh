...
# The integration.js is also in a git checkout containing my fork and branch of
# the Juris-M zotero code with my patches applied. The goal will be to have the
# changes become part of the upstream program, but for now, in order to avoid
# having people need to run from a git clone, I'll pull it here and
# monkey-patch it for them on top of the released version of Juris-M. That way,
# it's just a matter of installing a Firefox xpi package and they'll be able to
# start using the TeXmacs / Juris-M / Zotero integration.
#
